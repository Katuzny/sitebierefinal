...
//  foreach ($beerArray as $key => $value) {
  for ($i=0; $i < count($beerArray) ; $i++){
     echo "<div id = 'bloc" . $i . "' class = 'bloc'>" . "<br/>";
     for ($j=0; $j < count($beerArray[$i]) ; $j++){
        echo "<div class = 'par" . $j . "'>";
        switch ($j){
          case 1:
              // l'image
              echo "<img alt='apercu' class= 'image' src=" . $beerArray[$i][$j] . "/></div><br/>";
              break;
          case 3:
              //le prix
              $ttc = $beerArray[$i][$j] + $beerArray[$i][$j]*20/100;
              echo $ttc . " €</div>" . "<br/>";
              break;
          case 2:
              $texte = (String)$beerArray[$i][$j];
              $newtexte = substr($texte,0,strlen($texte)/2);
              echo (String)$newtexte . "...</div>" . "<br/>";
               break;
           default:
              // du texte
              echo (String)$beerArray[$i][$j] . "</div>" . "<br/>";
              break;
        }
      }
    echo "</div>" . "<br/>";
